...
<table border="0" width="100%" height="80%">
    <tr>
        <td align="center" class="aniscale">
            ...
            <img style="height: auto; width: 210px;" src="<?php get_template_directory_uri_admin(); ?>/images/logo.png"
                 class="img-responsive" alt="image post">

            <div style="font-size: 25px;margin: 20px 0px;font-weight: 700;color: #fff;"><?php echo get_config("site_description"); ?></div>

            <form class="entry-form" action="<?= get_uri() ?>/auth/signin/" method="post" role="form"
                  autocomplete="off">

                <input autocomplete="off" name="login" id="login" tabindex="1" class="form-control width-1"
                       placeholder="<?= lang_s("LOGIN_PLACEHOLDER") ?>"
                       value="<?= $getv['values']['login'] ?? '' ?>"
                       type="text">

                <input name="password" id="password" tabindex="2" class="form-control width-1"
                       placeholder="<?= lang_s("PASSWORD_PLACEHOLDER") ?>" type="password">

                <input name="submit" tabindex="4" class="btn btn-default btn-lg"
                       value="<?= lang_s("SIGNIN_SUBMIT_INPUT") ?>" type="submit"
                       style="height: 40px; width: 98px; font-size: 13px; font-weight: 600;">
